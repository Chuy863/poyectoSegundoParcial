    ...
    .exec((err, categorias) => {
        if(err){
            return res.status(400).json({
              ok: false,
              msg: 'Ocurrio un error al momento de listar las categorias',
              err
            });
        }
        res.json({
...
app.post('/categoria', (req, res) =>{
    ...
    cat.save((err, catDB) => {
        if(err){
            return res.status(400).json({
              ok: false,
              msg: 'Error al momento de  insertar una categorias',
              err
            });
        }
        res.json({
            ok: true,
            msg: 'Categorias insertada con exito',
            catDB
          });
    });
});
